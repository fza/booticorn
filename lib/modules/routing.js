...
module.exports = {
   ...
   * Setup routes.
   */
  setup: function setupRouting(app, options) {
    if (loading || loaded) {
      return;
    }
    ...
    var routingConfigPath = path.join(app.get('basePath'), 'routing'),
      mainRoutingFile = app.get('config').app.routingConfigFile || 'routing.yml';

    var routing = loadRoutes(routingConfigPath, mainRoutingFile);
    _.each(routing.nested, setupRoute.bind(null, app, options, app, '/', null));
