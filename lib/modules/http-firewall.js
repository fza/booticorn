...
_.extend(HttpFirewall.prototype, {
   ...
   * @param {object} server
   * @param {boolean} [alreadyListening]
   */
  addServer: function (server, alreadyListening) {
    var self = this;

    if (self.servers.indexOf(server) !== -1) {
      return;
    }

    self.servers.push(server);

    var enableFirewall = function enableFirewall() {
      ...
      self.logger.info('Successfully enabled HTTP firewall');
    };

    if (!alreadyListening) {
      server.once('listening', enableFirewall);
    } else {
      enableFirewall();
    }
  },
