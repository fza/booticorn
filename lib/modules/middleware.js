...
module.exports = {
   ...
   */
  load: function loadMiddleware(app, cb) {
    var self = this;
    ...
    var middlewareBasePath = path.join(__dirname, '..', 'middleware');

    recursiveReadDir(middlewareBasePath, function (readDirError, files) {
      if (readDirError) {
        cb(readDirError);
        return;
      }
      ...
      async.eachLimit(files, 5, function (filePath, taskCb) {
        ...
        _.each(priorities, function (scopePriorities, scope) {
          scopePriorities.sort();
        });

        app.get('logger').info('Loaded all middleware');
