...
function getControllerSetup(app, controllerPath) {
  controllerPath = path.join(app.get('config').basePath, 'controllers', controllerPath);
...
module.exports = {
  ...
  generateUrl: function generateUrl(app, routeName, params, checkRequirements) {
    return util.generateUrlFromPath(app, generatePath(app, routeName, params, checkRequirements));
  },
  ...
  setup: function setupRouting(app, config, options, logger, cb) {
    ...
    var routing = loadRoutes(path.join(config.basePath, 'routing'), options.routing.configFile);
