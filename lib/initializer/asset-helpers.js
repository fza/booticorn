...
module.exports = function initAssetsHelper(app, config, options, logger, cb) {
  ...
  // Setup an early (the first!) middleware which wraps `res.render()` so that the following
  // params are always available in the template context:
  // req, res, app, config (=`app.get('config')`)
