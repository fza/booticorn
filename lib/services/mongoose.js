...
function setupModels(db, app, config, options, logger, cb) {
  ...
  recursiveReadDir(path.join(config.basePath, 'models'), function (readDirError, files) {
    if (readDirError) {
      cb(readDirError);
    }
...
module.exports = function initMongoose(app, config, options, logger, cb) {
  ...
  try {
    ...
    cb(new Error('Unable to require mongoose. It must be defined as a dependency in your package.json.'));
  }
